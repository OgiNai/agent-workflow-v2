...
async def generate_code(request_data: GenerateSchema, credentials: HTTPAuthorizationCredentials = Depends(security)) -> Response:
    """
...
@router.post("/review")
async def review_code(request_data: ReviewSchema, credentials: HTTPAuthorizationCredentials = Depends(security)) -> Response:
    """
...
async def call_agent_system(request_data: ReviewSchema | GenerateSchema) -> Response:
    ...
    try:
        if isinstance(request_data, ReviewSchema):
            ...
            result = await agents_result(request_data.generate_content, request_data.generate_rounds)
        ...
        err_msg = "reviewing" if isinstance(request_data, ReviewSchema) else "generation"
        return JSONResponse(
